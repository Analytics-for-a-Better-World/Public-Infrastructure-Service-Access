...
def show_nodes_colored_per_component_with_basemap(
    ...
    print(f'[+] Found {n_components} components (largest at index {largest_group} with size {largest_size:,} nodes) in {pc() - t_comp:.2f}s')
    ...
    print(f'[+] Plotted {len(nodes):,} nodes in {pc() - t_plot:.2f}s')
    ...
    print(f'[+] Added basemap in {pc() - t_base:.2f}s')
