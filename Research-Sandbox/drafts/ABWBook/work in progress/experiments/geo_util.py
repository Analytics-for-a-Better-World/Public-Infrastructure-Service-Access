...
from colorcet import rgb_to_hex  # Convert (R, G, B) to hex
...
def get_default_logger(name: str = __name__) -> logging.Logger:
    '''
    Create and return a default logger for the given module name.

    If the logger does not already have handlers, it adds a StreamHandler
    that outputs to stdout with a timestamped format. The default log level is INFO.

    Parameters:
    - name: The name to identify the logger (defaults to the module’s `__name__`).
...
def load_or_acquire(
    ...
    **kwargs
    ...
    - acquire_func: Callable that returns the data when invoked with *args and **kwargs.
    - data_path: Path object indicating the directory for storing/loading cached data.
    - *args, **kwargs: Passed to `acquire_func`.
    - force_refresh: If True, skip loading and acquire data anew.
    ...
    if path.exists() and not force_refresh:
        ...
        duration = pc() - start
        if logger:
            logger.info(f'Data loaded in {duration:.2f} seconds')
        return data

    if logger:
        reason = 'forced refresh' if force_refresh else 'cache miss'
        logger.info(f'Acquiring new data due to {reason}, using {acquire_func.__name__}()')
    ...
    if logger:
        logger.info(f'Data acquired in {duration:.2f} seconds and saved to {path}')
