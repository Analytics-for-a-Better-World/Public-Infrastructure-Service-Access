...
def plot_month_calendar(
    ...
    *,
    ...
    line_gap: float = 0.14,
    show_weekday_in_cell: bool = False,
    fontsize_cell_weekday: int = 7,
    ...
    def wrapped_lines(texts: list[str]) -> list[str]:
        lines: list[str] = []
        for text in texts:
            wrapped = fill(text, width=wrap_width)
            lines.extend(wrapped.splitlines() or [""])
        return lines

    def limited_lines(lines: list[str], capacity: int | None) -> list[str]:
        if capacity is None:
            return lines
        if capacity <= 0:
            return []
        if len(lines) <= capacity:
            return lines
        return lines[: max(0, capacity - 1)] + ["..."]

    def row_iso_week(row_days: list) -> str:
        monday = next((d for d in row_days if d.weekday() == 0), row_days[0])
        return f"W{pd.Timestamp(monday).isocalendar().week}"
    ...
    def drawLane(
        ...
        *,
        ...
        if not lines:
            return
        lane_height = max(0.01, y_bottom - y_top)
        cell_height_inches = fig.get_size_inches()[1] / max(1, n_rows)
        lane_height_points = cell_height_inches * lane_height * 72
        font_size = min(float(fontsize_event), max(3.2, lane_height_points / (len(lines) * 1.18)))
        step = lane_height / max(1, len(lines))
        for i, line in enumerate(lines):
            y = y_top + i * step
            ax.text(x, y, line, fontsize=font_size, va='top', ha='left', color=color, clip_on=True)
    ...
    for (year, month), g in s.groupby(['year', 'month'], sort=True):
        ...
        ax.set_yticklabels([row_iso_week(week) for week in weeks])
        ax.set_xlim(-0.5, 6.5)
        ...
        for r in range(n_rows):
            for c in range(7):
                ...
                ax.text(c + 0.46, r - 0.36, str(day.day),
                        fontsize=fontsize_day, va='top', ha='right')
                if show_weekday_in_cell:
                    ax.text(c + 0.46, r - 0.36, day.strftime('%a'),
                            fontsize=fontsize_cell_weekday, va='top', ha='right', color='#555555')
                ...
                if autoFit:
                    ...
                    capM = capA = maxPerSlot * 3 if maxPerSlot is not None else None

                drawM = limited_lines(wrapped_lines(itemsM), capM)
                drawA = limited_lines(wrapped_lines(itemsA), capA)
        ...
        if file_name:
            out_base = Path(file_name)
            out_dir = out_base.parent
            if str(out_dir) == ".":
                out_path = Path(f"{year}_{month:02d}_{out_base.name}")
            else:
                out_dir.mkdir(parents=True, exist_ok=True)
                out_path = out_dir / f"{year}_{month:02d}_{out_base.name}"
