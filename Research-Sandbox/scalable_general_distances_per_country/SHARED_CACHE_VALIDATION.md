...
and the reengineered scalable architecture use one reproducible cache, not parallel hidden
state.
...
- Reengineered draft: `C:\local\GIT\Public-Infrastructure-Service-Access\Research-Sandbox\drafts\scalable`
...
matrix artifacts. The scalable architecture must not create a second real-data cache under
the scalable draft folder.
...
| case | status in scalable architecture | purpose |
|---|---|---|
...
| `timor_leste_health_agg8_10km` | not yet implemented as scalable case | max-cover input matrix |
| `vietnam_health_agg10_150km` | not yet implemented as scalable case | large max-cover input matrix |
| `nusa_tenggara_schools_candidates_2km` | not yet implemented as scalable case | table destinations and candidate sources |
| `luxembourg_aed_network_nodes` | not yet implemented as scalable case | network-node set covering |
...
- original command
- scalable command/notebook cell or adapter used
...
```yaml
schema_version: scalable-general-distances-manifest/v1
...
the current deduplicated scalable/original-cache path has 391. The missing earlier IDs are
