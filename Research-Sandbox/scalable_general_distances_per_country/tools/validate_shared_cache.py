...
RESEARCH_SANDBOX = scalable_ROOT.parents[1]
...
if str(scalable_ROOT / "src") not in sys.path:
    sys.path.insert(0, str(scalable_ROOT / "src"))
...
from scalable_distances.manifest import build_manifest, write_manifest  # noqa: E402
...
CASES: tuple[ValidationCase, ...] = (
    ...
    ValidationCase(
        ...
        reengineered_status="Not yet implemented as a scalable architecture case.",
        expected_summary={
...
def inventory(cases: list[ValidationCase], do_hash: bool = False) -> dict[str, Any]:
    payload: dict[str, Any] = {
        ...
        "scalable_root": str(scalable_ROOT),
...
def inventory_manifest(cases: list[ValidationCase], do_hash: bool = False) -> dict[str, Any]:
    ...
    return build_manifest(
        ...
        implementation={
            "name": "scalable_distances",
            "role": "reengineered",
            "root": str(scalable_ROOT),
        },
        code={},
        case={
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Shared-cache validation harness for original and scalable pipelines.")
    parser.add_argument(
        "action",
    ...
    if args.action == "manifest":
        ...
        if args.output is None:
            args.output = scalable_ROOT / "diagnostics" / "shared_cache_manifest.yaml"
