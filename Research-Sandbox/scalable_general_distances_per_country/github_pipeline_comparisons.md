...
| capability | GitHub main documented baseline | local pipeline | scalable draft architecture |
|---|---|---|---|
...
| use case | metric | earlier persisted output | current scalable architecture/cache | discrepancy |
|---|---:|---:|---:|---:|
...
## Comparison 5: scalable architecture Fit vs GitHub Pipeline

What the scalable draft improves:
...
- Full parity with local `run_pipeline.py` composable source/destination CLI is not yet implemented as a scalable runner API.
...
2. Add a scalable notebook case for table-as-source and table-as-destination using a small synthetic table to validate source/target role reuse beyond the unit-style basic notebook.
